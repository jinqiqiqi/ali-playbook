            ...
            steps {
                withCredentials ([sshUserPrivateKey(credentialsId: 'rootk', keyFileVariable: 'SSH_KEY_FILE', usernameVariable: 'SSH_USER')]) {
                    ...
                    ansiColor('xterm') {
                        ...
                        ansiblePlaybook credentialsId: 'rootk', disableHostKeyChecking: true, inventory: 'inventory/hosts', playbook: 'playbook.yml', colorized: true, extras: '-D -e host_name=common_group -e role_name=fixing  -e addition="${BUILD_URL}consuleFull"'
                    }
                }
            }
        }
